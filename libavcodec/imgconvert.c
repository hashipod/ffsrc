...
int avpicture_fill(AVPicture *picture, uint8_t *ptr, int pix_fmt, int width,
    ...
    case PIX_FMT_YUVJ444P:
        w2 =
            (width + (1 << pinfo->x_chroma_shift) - 1) >> pinfo->x_chroma_shift;
        h2 = (height + (1 << pinfo->y_chroma_shift) - 1) >>
             pinfo->y_chroma_shift;
        size2 = w2 * h2;
        picture->data[0] = ptr;
...
void ff_shrink44(uint8_t *dst, int dst_wrap, const uint8_t *src, int src_wrap,
    ...
    for (; height > 0; height--) {
        ...
        d = dst;
        for (w = width; w > 0; w--) {
            d[0] = (s1[0] + s1[1] + s1[2] + s1[3] + s2[0] + s2[1] + s2[2] +
                    s2[3] + s3[0] + s3[1] + s3[2] + s3[3] + s4[0] + s4[1] +
                    s4[2] + s4[3] + 8) >>
                   4;
            s1 += 4;
            s2 += 4;
...
typedef struct ConvertEntry {
    void (*convert)(AVPicture *dst, const AVPicture *src, int width,
                    int height);
} ConvertEntry;
...
static void img_convert_init(void) {
    ...
    convert_table[PIX_FMT_YUV420P][PIX_FMT_UYVY422].convert =
        yuv420p_to_uyvy422;

    convert_table[PIX_FMT_YUV422P][PIX_FMT_YUV422].convert = yuv422p_to_yuv422;
    convert_table[PIX_FMT_YUV422P][PIX_FMT_UYVY422].convert =
        yuv422p_to_uyvy422;

    convert_table[PIX_FMT_YUV444P][PIX_FMT_RGB24].convert = yuv444p_to_rgb24;

    convert_table[PIX_FMT_YUVJ420P][PIX_FMT_RGB555].convert =
        yuvj420p_to_rgb555;
    convert_table[PIX_FMT_YUVJ420P][PIX_FMT_RGB565].convert =
        yuvj420p_to_rgb565;
    ...
    convert_table[PIX_FMT_YUVJ420P][PIX_FMT_RGBA32].convert =
        yuvj420p_to_rgba32;
    ...
    convert_table[PIX_FMT_UYVY422][PIX_FMT_YUV420P].convert =
        uyvy422_to_yuv420p;
    convert_table[PIX_FMT_UYVY422][PIX_FMT_YUV422P].convert =
        uyvy422_to_yuv422p;
    ...
    convert_table[PIX_FMT_UYVY411][PIX_FMT_YUV411P].convert =
        uyvy411_to_yuv411p;
}
...
int img_convert(AVPicture *dst, int dst_pix_fmt, const AVPicture *src,
    ...
    if (is_yuv_planar(dst_pix) && is_yuv_planar(src_pix)) // YUV to YUV planar
    {
        ...
        // there must be filters for conversion at least from and to YUV444
        // format
        switch (xy_shift) {
        case 0x00:
        ...
        // if yuv color space conversion is needed, we do it here on the
        // destination image
...
no_chroma_filter: // try to use an intermediate format
    ...
    if (src_pix_fmt == PIX_FMT_YUV422 || dst_pix_fmt == PIX_FMT_YUV422) {
        int_pix_fmt =
            PIX_FMT_YUV422P; // specific case: convert to YUV422P first
        ...
        int_pix_fmt =
            PIX_FMT_YUV411P; // specific case: convert to YUV411P first
