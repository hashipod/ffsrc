        ...
        int whence)) // 在缓存中做seek操作(具体实现依赖于opaque的类型)
{
    ...
    if (!s->write_flag)
        s->buf_end = buffer; // 初始情况下，缓存中没有效数据，所以buf_end
                             // 指向缓存首地址。
    else
...
static void fill_buffer(ByteIOContext *s) {
    ...
    if (len <= 0) {
        ...
        // 如果是到达文件末尾就不要改buffer参数，这样不用重新读数据就可以做seek
        // back 操作。
        s->eof_reached = 1;
...
int get_byte(ByteIOContext *s) // NOTE: return 0 if EOF, so you cannot use it if
                               // EOF handling is necessary
{
...
int url_fopen(ByteIOContext *s, const char *filename, int flags) {
    ...
    if (max_packet_size) {
        buffer_size =
            max_packet_size; // no need to bufferize more than one packet
    } else {
